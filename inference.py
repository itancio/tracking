...
def constructBayesNet(gameState: hunters.GameState):
    ...
    edges.extend([(GHOST0, OBS0),
                  (GHOST1, OBS1),
                  (PAC, OBS0),
                  (PAC, OBS1)])
    # Add all possible position tuples of Pacman and the two ghosts
    locations = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]
    distanceAndNoise = range(X_RANGE + Y_RANGE + MAX_NOISE - 1)
    for var in variables[:3]:
        variableDomainsDict[var] = locations
    # Add all possible manhattan distance and noise
    for var in variables[3:]:
        variableDomainsDict[var] = distanceAndNoise
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # Loop over hidden variables in the order of elimination
        for hiddenVariable in eliminationOrder:
            # Perform joining Factors for each variable
            currentFactorsList, eliminationVariable = joinFactorsByVariable(currentFactorsList, hiddenVariable)
            ...
            currentFactorsList.append(eliminate(eliminationVariable, hiddenVariable))
        
        # Normalize the probabilities of the factor so the sum would add up to 1
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Handling special case for capturing a ghost
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # Loops over all legal positions and special jail positions
        for ghostPosition in self.allPositions:
            # Update beliefs using the observation probability
            self.beliefs[ghostPosition] *= self.getObservationProb(noisyDistance, 
                                                                   pacmanPosition, 
                                                                   ghostPosition, 
                                                                   jailPosition)
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPosition in self.particles:
            distribution[ghostPosition] += self.getObservationProb(noisyDistance, pacmanPosition, ghostPosition, jailPosition)
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        newPosDist = [self.getPositionDistribution(gameState, particle) for particle in self.particles]
        self.particles = [pos.sample() for pos in newPosDist]
        "*** END YOUR CODE HERE ***"
